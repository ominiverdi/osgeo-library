...
Usage:
    from db.chunking import chunk_text, chunk_pages
...
if __name__ == "__main__":
    ...
    print(
        f"Input length: {len(sample_text)} chars (~{estimate_tokens(sample_text)} tokens)"
    )
    print()
    ...
    # Test with actual extraction if available
    test_file = Path("db/data/sam3/pages/page_001.json")
    if test_file.exists():
        print("\nTesting with real extraction (sam3 page 1):")
        print("-" * 60)
        ...
        print(f"Page text: {len(text)} chars (~{estimate_tokens(text)} tokens)")
        print(f"Generated {len(chunks)} chunks")
        for chunk in chunks:
            print(
                f"  Chunk {chunk.chunk_index}: {len(chunk.content)} chars, "
                f'starts with "{chunk.content[:50]}..."'
            )
